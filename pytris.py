...
class ui_variables:
    ...
    font_path = Path(fonts_path, "OpenSans-Light.ttf")
    font_path_b = Path(fonts_path, "OpenSans-Bold.ttf")
    font_path_i = Path(fonts_path, "Inconsolata/Inconsolata.otf")
    ...
    click_sound = pygame.mixer.Sound(Path(sounds_path, "SFX_ButtonUp.wav"))
    move_sound = pygame.mixer.Sound(Path(sounds_path, "SFX_PieceMoveLR.wav"))
    drop_sound = pygame.mixer.Sound(Path(sounds_path, "SFX_PieceHardDrop.wav"))
    single_sound = pygame.mixer.Sound(Path(sounds_path, "SFX_SpecialLineClearSingle.wav"))
    double_sound = pygame.mixer.Sound(Path(sounds_path, "SFX_SpecialLineClearDouble.wav"))
    triple_sound = pygame.mixer.Sound(Path(sounds_path, "SFX_SpecialLineClearTriple.wav"))
    tetris_sound = pygame.mixer.Sound(Path(sounds_path, "SFX_SpecialTetris.wav"))
...
def get_mino_from_bag():
    global bag
    if bag == []:
        bag = [1, 2, 3, 4, 5, 6, 7]
    i = random.randrange(len(bag))      # get random index
    bag[i], bag[-1] = bag[-1], bag[i]   # swap with the last element
    return bag.pop()                    # pop last element O(1)
...
mino = get_mino_from_bag() # Current mino
next_mino = get_mino_from_bag() # Next mino
...
with open(Path(Path(__file__).parent.absolute(),"leaderboard.txt")) as f:
    lines = f.readlines()
